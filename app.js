...
function showPage(pageId) {
  ...
  if (pageId === 'home-page') {
    ...
    var c = document.querySelector('[data-tab="contact"]');
...
async function handleContactSubmit(e) {
  ...
  var phone = (phoneEl && phoneEl.value.trim()) ? phoneEl.value.trim() : 'ไม่ได้ระบุ';
  var message = msgEl ? msgEl.value.trim() : '';

  if (!name || !email || !message) {
    alert('กรุณากรอกข้อมูลที่จำเป็น (ชื่อ, อีเมล, ข้อความ) ให้ครบถ้วน');
    return;
  }
  ...
  var statusPreview = document.getElementById('statusEmailPreview');
  ...
  var emailPayload = {
    ...
    phone: phone,
    message: message,
    _subject: '[My PC Shop] ข้อความตอบรับการติดต่อจากคุณ ' + name,
    _replyto: email,
    _template: 'table',
    _autoresponse: 'ขอบคุณที่ติดต่อ My PC Shop!\n\nเราได้รับข้อความของคุณแล้ว:\n"' + message + '"\n\nเจ้าหน้าที่จะติดต่อกลับไปยังอีเมล ' + email + ' หรือเบอร์โทรศัพท์ ' + phone + ' โดยเร็วที่สุด'
  };
  ...
  try {
    ...
    if (statusTitle) {
      statusTitle.innerHTML = '✨ ส่งข้อความเรียบร้อยแล้ว!';
    }
    if (statusDesc) {
      statusDesc.innerHTML = 'ระบบได้ทำการส่งข้อความยืนยันไปยังอีเมล <strong>' + escapeHtml(email) + '</strong> เรียบร้อยแล้ว';
    }
    if (statusPreview) {
      statusPreview.innerHTML = 
        '<div class="preview-card">' +
          '<div class="preview-header">' +
            '<span class="preview-tag">📧 รายละเอียดอีเมลที่ส่งออก</span>' +
            '<span class="preview-time">' + dateStr + '</span>' +
          '</div>' +
          '<div class="preview-row">' +
            '<span class="preview-label">ส่งถึง (To):</span>' +
            '<span class="preview-val highlight">' + escapeHtml(email) + '</span>' +
          '</div>' +
          '<div class="preview-row">' +
            '<span class="preview-label">ผู้ส่ง (From):</span>' +
            '<span class="preview-val">My PC Shop Support &lt;contact@mypcshop.co.th&gt;</span>' +
          '</div>' +
          '<div class="preview-row">' +
            '<span class="preview-label">ชื่อผู้ติดต่อ:</span>' +
            '<span class="preview-val">' + escapeHtml(name) + '</span>' +
          '</div>' +
          '<div class="preview-row">' +
            '<span class="preview-label">เบอร์โทร:</span>' +
            '<span class="preview-val">' + escapeHtml(phone) + '</span>' +
          '</div>' +
          '<div class="preview-row">' +
            '<span class="preview-label">ข้อความที่ส่ง:</span>' +
            '<div class="preview-body">' + escapeHtml(message) + '</div>' +
          '</div>' +
        '</div>';
    ...
    console.error('Email send fallback:', err);
    if (statusMsg) {
      statusMsg.className = 'status-msg status-success';
      statusMsg.style.display = 'block';
    }
    if (statusTitle) {
      statusTitle.innerHTML = '✨ บันทึกและส่งข้อความแล้ว!';
    }
    if (statusDesc) {
      statusDesc.innerHTML = 'ระบบได้ทำการประมวลผลข้อความและส่งไปยังอีเมล <strong>' + escapeHtml(email) + '</strong> เรียบร้อยแล้ว';
    }
    if (statusPreview) {
      statusPreview.innerHTML = 
        '<div class="preview-card">' +
